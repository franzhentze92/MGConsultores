...
import { IconReportMoney, IconBuildingBank, IconPigMoney, IconChartBar, IconShieldCheck, IconUserCheck, IconCoins, IconCalculator, IconBriefcase, IconReceipt2, IconTrendingUp, IconUsersGroup, IconCertificate, IconFileInvoice, IconCashBanknote, IconCreditCard, IconChecklist, IconAward } from "@tabler/icons-react";
...
const values = [
  ...
  { icon: <IconCoins className="w-8 h-8 text-blue-700 animate-pulse" />, label: "Integridad" },
  ...
  <AppLayout>
    ...
    <section className="relative w-full min-h-[80vh] flex flex-col md:flex-row items-center justify-between px-6 md:px-16 py-20 bg-gradient-to-br from-blue-900 via-blue-800 to-blue-600 overflow-hidden">
      ...
      <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 1, delay: 0.5 }} className="z-20 flex-1 flex items-center justify-center mt-10 md:mt-0">
        <img src="https://images.unsplash.com/photo-1556740772-1a741367b93e?auto=format&fit=crop&w=600&q=80" alt="Contabilidad y Finanzas" className="w-96 h-96 object-cover rounded-3xl shadow-2xl border-4 border-blue-100" />
      </motion.div>
    </section>

    {/* About Preview */}
    <section className="w-full py-16 bg-white flex flex-col md:flex-row items-center justify-center md:gap-16 gap-8 px-4 max-w-6xl mx-auto">
      <div className="flex-1 flex flex-col items-center md:items-start justify-center min-w-[300px]">
      ...
      <motion.img src="https://images.unsplash.com/photo-1551836022-4c4c79ecde16?auto=format&fit=crop&w=500&q=80" alt="Contabilidad y Finanzas" initial={{ opacity: 0, scale: 0.8 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 0.7 }} className="w-96 h-96 object-cover rounded-2xl shadow-lg flex-1" style={{ minWidth: '260px', maxWidth: '400px' }} />
    </section>
